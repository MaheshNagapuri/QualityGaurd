...
_MODELS_TMP = Path("/tmp/qualityguard_models")
_REQUIRED_PKLS = [
...
def _resolve_models_dir(base: Path) -> Path | None:
    """Return the first directory that contains all required pkl files, or None."""
    for candidate in [base / "ml" / "models", _MODELS_TMP]:
        if all((candidate / f).exists() for f in _REQUIRED_PKLS):
            return candidate
    return None
...
def _eval_model(model_pkl, scaler_pkl, encoder_pkl, eval_csv, target_col):
    ...
    models_dir = Path(os.environ.get("QUALITYGUARD_MODELS_DIR",
                                     str(root / "ml" / "models")))
    try:
        model   = pickle.load(open(models_dir / model_pkl,   "rb"))
        scaler  = pickle.load(open(models_dir / scaler_pkl,  "rb"))
        encoder = pickle.load(open(models_dir / encoder_pkl, "rb"))
